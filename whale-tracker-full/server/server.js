...
/* --- (optioneel) UI statisch serveren vanaf /public ---
   Laat weg als je UI op Netlify/Surge blijft staan.
   Zet aan door de volgende regel te de-commenten:
*/
...
/* ------------ Whale Alert proxy (optioneel, voor koop/verkoop-bias) ------------ */
...
app.get('/api/nokey/btc/transactions', async (req, res) => {
  try {
    ...
    const price = (await (await fetch('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin&vs_currencies=usd')).json())?.bitcoin?.usd || 0;
    const txs = [];
    for (const b of useBlocks) {
      ...
      for (const tx of arr) {
        try {
          ...
          let sum = 0; for (const o of outs) sum += Number(o?.value || 0);
          const usd = sum * price;
          if (usd >= minUsd) txs.push({ timestamp: b.timestamp, hash: tx.txid||'', amount_usd: usd, from:{owner_type:'unknown'}, to:{owner_type:'unknown'}, chain:'btc' });
        } catch {}
      }
    }
    res.setHeader('Access-Control-Allow-Origin','*'); res.json({ transactions: txs });
  } catch (e) { res.status(500).json({ error: String(e) }); }
});
...
app.get('/api/nokey/xrp/transactions', async (req, res) => {
  try {
    const minUsd = Number(req.query.min_usd || '500000');
    const price = (await (await fetch('https://api.coingecko.com/api/v3/simple/price?ids=ripple&vs_currencies=usd')).json())?.ripple?.usd || 0;
    ...
    for (const it of (data?.transactions || [])) {
      ...
      if (typeof amt === 'string') {
        ...
        const usd = xrp * price;
        if (usd >= minUsd) out.push({ timestamp: Math.floor(new Date(it.date).getTime()/1000), hash: it.hash, amount_usd: usd, from:{owner_type:'unknown'}, to:{owner_type:'unknown'}, chain:'xrp' });
      }
    }
    res.setHeader('Access-Control-Allow-Origin','*'); res.json({ transactions: out });
  } catch (e){ res.status(500).json({ error: String(e) }); }
});
...
app.get('/api/nokey/ltc/transactions', async (req, res) => {
  try {
    ...
    const price = (await (await fetch('https://api.coingecko.com/api/v3/simple/price?ids=litecoin&vs_currencies=usd')).json())?.litecoin?.usd || 0;
    const txs = [];
    for (const b of (blocks||[]).slice(0, limitBlocks)){
      const arr = await (await fetch(`https://litecoinspace.org/api/block/${b.id}/txs/0`)).json();
      for (const tx of (arr||[])){
        try { const outs = Array.isArray(tx.vout)?tx.vout:[]; let sum=0; for (const o of outs) sum+=Number(o?.value||0); const usd=sum*price; if (usd>=minUsd) txs.push({timestamp:b.timestamp, hash:tx.txid||'', amount_usd:usd, from:{owner_type:'unknown'}, to:{owner_type:'unknown'}, chain:'ltc'});} catch{}
      }
    }
    res.setHeader('Access-Control-Allow-Origin','*'); res.json({ transactions: txs });
  } catch(e){ res.status(500).json({ error: String(e) }); }
...
app.get('/api/nokey/doge/transactions', async (req,res)=>{
  try{
    const minUsd = Number(req.query.min_usd || '500000');
    const price = (await (await fetch('https://api.coingecko.com/api/v3/simple/price?ids=dogecoin&vs_currencies=usd')).json())?.dogecoin?.usd || 0;
    ...
    for (const it of arr){
      ...
      const usd = val * price;
      if (usd >= minUsd) txs.push({ timestamp: it?.time || Math.floor(Date.now()/1000), hash: it?.hash || it?.transaction_hash || '', amount_usd: usd, from:{owner_type:'unknown'}, to:{owner_type:'unknown'}, chain:'doge' });
    }
    res.setHeader('Access-Control-Allow-Origin','*'); res.json({ transactions: txs });
  }catch(e){ res.status(500).json({ error:String(e) }); }
});
...
app.get('/api/nokey/bch/transactions', async (req,res)=>{
  try{
    const minUsd = Number(req.query.min_usd || '500000');
    const price = (await (await fetch('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin-cash&vs_currencies=usd')).json())?.['bitcoin-cash']?.usd || 0;
    ...
    for (const it of arr){
      const val = Number(it?.value || it?.transaction?.value || 0);
      const usd = val * price;
      if (usd >= minUsd) txs.push({ timestamp: it?.time || Math.floor(Date.now()/1000), hash: it?.hash || it?.transaction_hash || '', amount_usd: usd, from:{owner_type:'unknown'}, to:{owner_type:'unknown'}, chain:'bch' });
    }
    res.setHeader('Access-Control-Allow-Origin','*'); res.json({ transactions: txs });
  }catch(e){ res.status(500).json({ error:String(e) }); }
});
...
app.get('/api/nokey/xlm/payments', async (req, res) => {
  try {
    const minUsd = Number(req.query.min_usd || '500000');
    const price = (await (await fetch('https://api.coingecko.com/api/v3/simple/price?ids=stellar&vs_currencies=usd')).json())?.stellar?.usd || 0;
    ...
    for (const rec of recs){
      ...
      if (usd >= minUsd) out.push({ timestamp: Math.floor(new Date(rec.created_at).getTime()/1000), hash: rec.transaction_hash || '', amount_usd: usd, from:{owner_type:'unknown'}, to:{owner_type:'unknown'}, chain:'xlm' });
    }
    res.setHeader('Access-Control-Allow-Origin','*'); res.json({ transactions: out });
  } catch(e){ res.status(500).json({ error:String(e) }); }
});
...
app.get('/api/nokey/evm/transactions', async (req,res)=>{
  try{
    ...
    const price = (await (await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${encodeURIComponent(geckoId)}&vs_currencies=usd`)).json())?.[geckoId]?.usd || 0;
    ...
    for (let i=latest; i>start; i--){
      ...
      const block = bJson?.result; if (!block) continue;
      const ts = parseInt(block.timestamp || '0x0', 16);
      for (const t of (block.transactions||[])){
        try{
          const valWei = BigInt(t.value || '0x0'); if (valWei === 0n) continue;
...
app.get('/api/nokey/sol/transactions', async (req, res) => {
  try {
    ...
    const price = (await (await fetch('https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd')).json())?.solana?.usd || 0;
    const headers = { 'content-type': 'application/json' };
    ...
    for (let s = latestSlot; s > startSlot; s--){
      ...
      const bJson = await bRes.json();
      const block = bJson?.result; if (!block?.transactions) continue;
      const ts = block.blockTime || Math.floor(Date.now()/1000);
      for (const tx of block.transactions){
        try{
          const meta = tx?.meta; if (!meta?.preBalances || !meta?.postBalances) continue;
          ...
          const usd = sol * price;
          if (usd >= minUsd) txs.push({ timestamp: ts, hash: tx?.transaction?.signatures?.[0]||'', amount_usd: usd, from:{owner_type:'unknown'}, to:{owner_type:'unknown'}, chain:'sol' });
        }catch{}
      }
      if (txs.length > 500) break;
    }
...
app.get('/api/nokey/trx/transactions', async (req, res) => {
  try {
    const minUsd = Number(req.query.min_usd || '500000');
    const price = (await (await fetch('https://api.coingecko.com/api/v3/simple/price?ids=tron&vs_currencies=usd')).json())?.tron?.usd || 0;
    ...
    for (const it of arr){
      ...
      if (usd >= minUsd) txs.push({ timestamp: Math.floor((it.timestamp||Date.now())/1000), hash: it.hash || '', amount_usd: usd, from:{owner_type:'unknown'}, to:{owner_type:'unknown'}, chain:'trx' });
    }
    res.setHeader('Access-Control-Allow-Origin','*'); res.json({ transactions: txs });
  } catch(e){ res.status(500).json({ error:String(e) }); }
});
...
app.get('/api/nokey/near/transactions', async (req, res) => {
  try {
    ...
    const price = (await (await fetch('https://api.coingecko.com/api/v3/simple/price?ids=near&vs_currencies=usd')).json())?.near?.usd || 0;
    const headers = { 'content-type': 'application/json' };
    ...
    for (let h = latestHeight; h > start; h--){
      ...
      const block = blkJson?.result; if (!block) continue;
      ...
      for (const ch of (block.chunks||[])){
        ...
        const chunk = cJson?.result; if (!chunk?.transactions) continue;
          ...
          try{
            for (const a of (t.actions||[])){
              if (a?.Transfer?.deposit){
                ...
                if (usd >= minUsd) txs.push({ timestamp: ts, hash: t.hash||'', amount_usd: usd, from:{owner_type:'unknown'}, to:{owner_type:'unknown'}, chain:'near' });
              }
            }
          }catch{}
        }
...
/* --- (optioneel) Catch-all naar index.html als je UI óók via Render serveert --- */
// app.get('*', (_req, res) => {
